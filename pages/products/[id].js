...
export default function Home() {
  ...
  const [checkbrand, setCheckBrand] = useState({ value: null });
  const [checkCat, setCheckCat] = useState({ value: query.id });
  ...
  const fetchBrands = useCallback(async () => {
    try {
      ...
      if (response.status === 200) {
        const data = response.data && response.data.body.brand;
        data.map((item) => {
          return {
            name: item.name,
            active: false,
  ...
  const fetchProducts = useCallback(async (id) => {
    try {
      const response = await Requests.Products.CategoryWiseProduct(id);
      if (response.status === 200) {
                ...
                products.map((item, index) => {
                  return (
                    <div
                      ...
                      key={index}
                    >
                      <ProductShow handleBusket={handleBusket} item={item} />
                    </div>
                  );
                })}
            </Container.Row>
          ...
          <div className="d-flex justify-content-between border pt-2 pb-2">
            <div className="d-flex justify-content-start p-2">
              ...
              {new Array(7).fill().map((_, index) => {
                return (
                  <span
                    className="text-muted fs-14 mb-0 pe-1"
                    key={index}
                    style={
                      index === 1
                        ? { color: "#f6990e !important" }
            ...
            <div className="bg-primary">
              <Text className="fs-14 text-secondary mb-0 p-2 text-white">
                +12 more products
              </Text>
            </div>
            <div>
              <Text className="fs-14 text-secondary mb-0 p-2">
