...
import { getODPDatabaseCart } from "../../utils/utilities";
import { useEffect, useState } from "react";

export const ODPCart = (props) => {
  const [cartitems, setItems] = useState([]);

  useEffect(() => {
    setItems(getODPDatabaseCart());
  ...
  return (
    <div className=" w-100 mt-2 ms-2 p-2">
      ...
      <div className="d-flex justify-content-between">
        <div className="w-50">
          <Text className="fs-16 text-muted">On Demand Printing</Text>
        </div>
        ...
        return (
          <>
            <CartItem
              ...
              length={index}
              item={item}
              value={Object.values(getODPDatabaseCart())[index]}
            />
          </>
        );
      })}
