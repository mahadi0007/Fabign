...
const Index = (props) => {
  ...
  const onSubmit = async (data) => {
    ...
    if (!token) {
      ...
      try {
        ...
        const response = await Requests.Measurement.ProfileStore(formData);
  ...
  const fetchMeasurementData = useCallback(async (id) => {
    try {
      const response = await Requests.Measurement.Index(id);
  ...
  const fetchSizeProfile = useCallback(async () => {
    try {
      const response = await Requests.Measurement.ProfileIndex();
      if (response.status === 200) {
