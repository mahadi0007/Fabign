    ...
    return (
        <div className=' w-100 mt-2 ms-2 p-2'>
            ...
            <div className='d-flex justify-content-between'>
                <div className='w-50'>
                    <Text className="fs-16 text-muted">E-Commerce</Text>
                </div>
            ...
            {Object.keys(cartitems).map((item, index) => {
                return (
                    <>
                        <CartItem getCart={props.getCart} handleRemoveCart={props.handleRemoveCart} key={index} length={index} item={item} value={Object.values(getDatabaseCart())[index]} />
                    </>
                )
            })}
        </div>
